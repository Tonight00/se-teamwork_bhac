...
public class edit_info extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edit_btn_sure.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(have_username == 0  && have_phoneNum == 0 && hava_email == 0 && have_lastName == 0 && have_firstName == 0 && have_studentId == 0 && have_gender == 3 ){
                    Toast.makeText(edit_info.this, "您还未进行任何修改", Toast.LENGTH_SHORT).show();
                }else
                if(have_username == 1 && !check_username(username)){
    ...
    public void put_edit_info(int have_username,int have_phoneNum,int hava_email,int have_lastName,int have_firstName,int have_studentId,int have_gender){
        ...
        call.enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Looper.prepare();
                Toast.makeText(edit_info.this, "未连接到互联网 put 失败", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
    ...
    public void get_yuxianshi(){
        ...
        call.enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Looper.prepare();
                Toast.makeText(edit_info.this, "未连接到互联网 get 失败", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(responseBodyJSONObject.get("username")!=null){
                            String temp2 = responseBodyJSONObject.get("username").toString();
                            edit_username.setHint(temp2.substring(1,temp2.length()-1));
                        }
                        if(responseBodyJSONObject.get("phoneNum")!=null){
                            String temp2 = responseBodyJSONObject.get("phoneNum").toString();
                            edit_phoneNum.setHint(temp2.substring(1,temp2.length()-1));
                        }
                        if(responseBodyJSONObject.get("email")!=null){
                            String temp2 = responseBodyJSONObject.get("email").toString();
                            edit_email.setHint(temp2.substring(1,temp2.length()-1));
                        }
                        if(responseBodyJSONObject.get("firstName")!=null){
                            String temp2 = responseBodyJSONObject.get("firstName").toString();
                            edit_firstName.setHint(temp2.substring(1,temp2.length()-1));
                        }
                        if(responseBodyJSONObject.get("lastName")!=null){
                            String temp2 = responseBodyJSONObject.get("lastName").toString();
                            edit_lastName.setHint(temp2.substring(1,temp2.length()-1));
                        }
                        if(responseBodyJSONObject.get("studentId")!=null){
                            String temp2 = responseBodyJSONObject.get("studentId").toString();
                            edit_studentId.setHint(temp2.substring(1,temp2.length()-1));
                        }
                        if(responseBodyJSONObject.get("gender")!=null){

                            String temp = responseBodyJSONObject.get("gender").toString();
                            if(temp.equals("0")){
                                edit_genderswitch.check(R.id.edit_unknown);
                            }else if (temp.equals("1")){
                                edit_genderswitch.check(R.id.edit_man);
                            }else{
                                edit_genderswitch.check(R.id.edit_woman);
                            }
