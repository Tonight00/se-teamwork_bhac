...
public class edit_huodong extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //获取数据库中这个活动的三个时间 给时间选择器加初始值
        GetActivity_yushe(select_which_activity.getid());
    ...
    public void get_getTags(){
        ...
        call.enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Looper.prepare();
                Toast.makeText(edit_huodong.this, "未连接到互联网 get 失败", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
    ...
    public void put_editActInfo(int tag){
        ...
        call.enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                Looper.prepare();
                Toast.makeText(edit_huodong.this, "未连接到互联网 post2 失败", Toast.LENGTH_SHORT).show();
                Looper.loop();
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                Looper.prepare();
                //Toast.makeText(edit_huodong.this, "未连接到互联网 post2 成功", Toast.LENGTH_SHORT).show();
    ...
    private void initTimerPicker() {
        ...
        //edit_huodong_ddl_selected_time.setText(time1);
        //edit_huodong_begin_selected_time.setText(time2);
        //edit_huodong_end_selected_time.setText(time3);
    ...
    public void GetActivity_yushe(int id){
        OkHttpClient client = new OkHttpClient();
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(responseBodyJSONObject.get("title")!=null){//
                            ...
                            edit_huodong_title.setText(title);
                        }

                        if(responseBodyJSONObject.get("ddl")!=null){
                            time1 = responseBodyJSONObject.get("ddl").getAsString();

                        }
                        if(responseBodyJSONObject.get("begin")!=null){
                            time2 = responseBodyJSONObject.get("begin").getAsString();

                        }
                        if(responseBodyJSONObject.get("end")!=null){
                            time3 = responseBodyJSONObject.get("end").getAsString();

                        }
                        edit_huodong_ddl_selected_time.setText(time1.substring(0,time1.length()-3));
                        edit_huodong_begin_selected_time.setText(time2.substring(0,time1.length()-3));
                        edit_huodong_end_selected_time.setText(time3.substring(0,time1.length()-3));


                        if(responseBodyJSONObject.get("brief")!=null){//
                            String brief = responseBodyJSONObject.get("brief").getAsString();
                            edit_huodong_brief.setText(brief);
                        }
                        ...
                        if(responseBodyJSONObject.get("limitPeopleNum")!=null){//
                            ...
                            if(limitPeopleNum.equals("-1")){
                                edit_huodong_limitp.setText("无限制");
                            }else{
                                edit_huodong_limitp.setText(limitPeopleNum);
                            }

                        }
                        if(responseBodyJSONObject.get("place")!=null){//
                            String place = responseBodyJSONObject.get("place").getAsString();
                            edit_huodong_place.setText(place);
                        }
